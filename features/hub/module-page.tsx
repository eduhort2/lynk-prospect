...
function formatValue(value: unknown, format?: HubColumn["format"]) {
  ...
  if (format === "date") return new Intl.DateTimeFormat("pt-BR").format(new Date(String(value)));
  return String(value);
}

export function HubModulePage({ table, title, eyebrow, description, singular, fields, columns, searchKeys = [], defaultOrder = "created_at" }: HubModulePageProps) {
  const { organizationId } = useOrganization();
  const supabase = useMemo(() => createBrowserSupabase(), []);
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<Record<string, string | number>>(() => Object.fromEntries(fields.map((field) => [field.key, field.defaultValue ?? ""])));

  const query = useQuery({
    queryKey: ["hub-module", table, organizationId],
    enabled: Boolean(organizationId),
    queryFn: async () => {
      const { data, error } = await supabase.from(table).select("*").eq("organization_id", organizationId!).order(defaultOrder, { ascending: false });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = Object.fromEntries(Object.entries({ ...form, organization_id: organizationId }).map(([key, value]) => [key, value === "" ? null : value]));
    ...
    onSuccess: async () => {
      ...
      setOpen(false);
      setForm(Object.fromEntries(fields.map((field) => [field.key, field.defaultValue ?? ""])));
    },
      ...
      <Dialog open={open} onClose={() => setOpen(false)} title={`Novo ${singular.toLowerCase()}`} description="Os dados ficam vinculados à organização atual." size="lg">
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            {fields.map((field) => <div key={field.key} className={field.type === "textarea" ? "sm:col-span-2" : ""}><Label htmlFor={`hub-${field.key}`}>{field.label}{field.required ? " *" : ""}</Label>{field.type === "select" ? <Select id={`hub-${field.key}`} value={String(form[field.key] ?? "")} onChange={(event) => setForm((current) => ({ ...current, [field.key]: event.target.value }))}><option value="">Selecione</option>{field.options?.map((option) => <option key={option} value={option}>{option}</option>)}</Select> : field.type === "textarea" ? <Textarea id={`hub-${field.key}`} value={String(form[field.key] ?? "")} onChange={(event) => setForm((current) => ({ ...current, [field.key]: event.target.value }))} placeholder={field.placeholder} /> : <Input id={`hub-${field.key}`} type={field.type === "number" ? "number" : field.type || "text"} value={String(form[field.key] ?? "")} onChange={(event) => setForm((current) => ({ ...current, [field.key]: field.type === "number" ? Number(event.target.value) : event.target.value }))} placeholder={field.placeholder} />}</div>)}
          </div>
          <div className="flex justify-end gap-2 border-t border-line pt-5"><Button type="button" variant="ghost" onClick={() => setOpen(false)}>Cancelar</Button><Button type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? "Salvando..." : "Salvar"}</Button></div>
        </form>
      </Dialog>
    </>
  );
