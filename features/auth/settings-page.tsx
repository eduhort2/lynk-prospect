...
import { Bot, Building2, CheckCircle2, CircleUserRound, CloudUpload, MessageCircle, Save, ShieldCheck } from "lucide-react";
...
export function SettingsPage() {
  ...
  return (
    <>
      ...
      <Card className="mt-5"><CardHeader><div><CardTitle>Integrações futuras</CardTitle><CardDescription>Arquitetura preparada; nenhuma integração paga está ativa nesta versão.</CardDescription></div><Badge tone="amber">Fase 3</Badge></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[{ icon: Bot, name: "IA e código", text: "Geração de landing pages" }, { icon: MessageCircle, name: "WhatsApp Cloud API", text: "Mensagens e respostas oficiais" }, { icon: CloudUpload, name: "Deploy automático", text: "GitHub, Netlify e Vercel" }, { icon: CircleUserRound, name: "Cobrança SaaS", text: "Planos e limites por empresa" }].map(({ icon: Icon, name, text }) => <div key={name} className="rounded-xl border border-line bg-[#090909] p-4"><div className="mb-4 flex items-center justify-between"><Icon className="h-5 w-5 text-primary-light" /><CheckCircle2 className="h-4 w-4 text-zinc-700" /></div><p className="text-sm font-medium text-zinc-200">{name}</p><p className="mt-1 text-xs text-zinc-600">{text}</p></div>)}</div></CardContent></Card>
    </>
  );
}
