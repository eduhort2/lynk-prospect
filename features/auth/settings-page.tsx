...
export function SettingsPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Administração" title="Configurações" description="Gerencie seu perfil, a organização e acompanhe as integrações da operação LYNK." />
      ...
      <Card className="mt-5"><CardHeader><div><CardTitle>Plataforma e integrações</CardTitle><CardDescription>Chaves ficam somente no ambiente seguro da hospedagem.</CardDescription></div><Badge tone="green">v3.0</Badge></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[{ icon: MapPinned, name: "OpenStreetMap", text: "Prospecção gratuita e pública" }, { icon: MessageCircle, name: "WhatsApp Cloud API", text: "Envio + webhook de respostas" }, { icon: CloudUpload, name: "Gemini Flash-Lite", text: "Prompts com baixo custo" }, { icon: CreditCard, name: "E-mail LYNK", text: "Envio via lynkhq.com.br" }].map(({ icon: Icon, name, text }) => <div key={name} className="rounded-lg border border-line bg-[#0D0E0C] p-4"><Icon className="mb-4 h-5 w-5 text-primary" /><p className="text-sm font-medium text-zinc-200">{name}</p><p className="mt-1 text-xs text-zinc-600">{text}</p></div>)}</div></CardContent></Card>
    </>
  );
}
